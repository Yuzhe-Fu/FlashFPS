...
class ThreeNN(Function):
    ...
    def forward(ctx, unknown: torch.Tensor, known: torch.Tensor, stage: int = None) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        if stage < -40:
            # raise Error, this branch is from FractalCloud implementation, won't be used in FlashFPS project.
            raise NotImplementedError("wrong code execution path in upsampling.py-ThreeNN")
        else:
            # original 3NN

...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor, stage: int = None) -> torch.Tensor:
        ...
        if stage <-20:
            # this branch is from FractalCloud implementation, won't be used in FlashFPS project.
            raise NotImplementedError("wrong code execution path in group.py-BallQuery")
            ...
            # global GROUP_COUNT_1D
            # logging.info(f'BQ-{GROUP_COUNT_1D}: xyz:{xyz.shape}, new_xyz:{new_xyz.shape}, radius:{radius}, nsample:{nsample}, stage:{stage}')
            # GROUP_COUNT_1D += 1
        # pdb.set_trace()
        return idx

...
import sys
import os
fp = os.path.abspath(os.path.join(__file__, "../../../FPScache.py"))
sys.path.append(os.path.dirname(fp))
import FPScache
import numpy as np
from . import flashfps_runtime as _ffps
...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        self.grouper = create_grouper(0, group_args)
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            if not self.all_aggr:
                # FPScache.cache = None
                useFlashFPS = _ffps.useFlashFPS
                useFPS_Prune = _ffps.useFPS_Prune
                useFPS_Cache = _ffps.useFPS_Cache
                PruneRate = 1 - _ffps.PruneRate
                enable_quick_fps = _ffps.enable_quick_fps

                if useFlashFPS:
                    if FPScache.cache is None:
                        if useFPS_Prune: # Apply FPS-Prune
                            idx = self.sample_fn(p, p.shape[1]//self.stride, PruneRate, 1).long()
                            new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
                            FPScache.cache = new_p.clone()
                        else: # original FPS
                            idx = self.sample_fn(p, p.shape[1]//self.stride, 0, -100).long()
                            new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
                            FPScache.cache = new_p.clone()
                    else:
                        if useFPS_Cache: # Apply FPS-Cache
                            #FPSCACHE is [B,N,3], now just truncate it into [B,:p.shape[1] // self.stride,:]
                            npoints = p.shape[1] // self.stride
                            new_p = FPScache.cache[:, :npoints, :]
                            # FPScache.cache = new_p.clone()
                            # idx is [B, npoints], where the value is from 0 to p.shape[1] - 1
                            idx = torch.arange(0, npoints, device=p.device).unsqueeze(0).expand(p.shape[0], -1).contiguous()
                        else: # original FPS
                            idx = self.sample_fn(p, p.shape[1] // self.stride, 0, -100).long()
                            new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))          

                elif enable_quick_fps:
                    
                    idx_quickfps = self.sample_fn(p, p.shape[1] // self.stride, PruneRate, -100, enable_quick_fps).long()
                    idx = idx_quickfps[..., -1].unsqueeze(0)
                    new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
                else: # original FPS
                    idx = self.sample_fn(p, p.shape[1] // self.stride, 0, -100).long()
                    new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
                ...
                # fps_time2 = time.time()
                # fps_time = fps_time2 - fps_time1
                # logging.info(f'FPS time in ms: {fps_time * 1000} ms')
                # pdb.set_trace()
            else:
                new_p = p
            """ DEBUG neighbor numbers. 
            ...
            if self.use_res or 'df' in self.feature_type:
                    ...
                    f, -1, idx.unsqueeze(1).expand(-1, f.shape[1], -1)).contiguous()
                if self.use_res:
            ...
            p = new_p
        return p, f
        # return p, f, fps_time # only for segmentation, not for clf

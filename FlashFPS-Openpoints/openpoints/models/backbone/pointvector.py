    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            if not self.all_aggr:
                ...
                # fps_time1 = time.time()
                # print(f'p.shape: {p.shape}, stride: {self.stride}, useFlashFPS: {useFlashFPS}, useFPS_Prune: {useFPS_Prune}')
                if useFlashFPS:
                    if FPScache.cache is None:
                        if useFPS_Prune:
                            ...
                            idx = self.sample_fn(p, p.shape[1] // self.stride, PruneRate, -100).long()
                            new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
                            FPScache.cache = new_p.clone()
                            # print(f'p.shape: {p.shape}, stride: {self.stride}, useFlashFPS: {useFlashFPS}, useFPS_Prune: {useFPS_Prune}')
                        ...
                        if useFPS_Cache:
                            #FPSCACHE is [B,N,3], now just truncate it into [B,:p.shape[1] // self.stride,3]
                            # print('in FPS-Cache')
                            ...
                            idx = self.sample_fn(p, p.shape[1] // self.stride, self.stride, -100).long()
                            new_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))               
